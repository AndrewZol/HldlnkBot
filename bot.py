...
"""
    ...
    if goals:
        prompt += f"""ЦЕЛИ ПО КБЖУ НА ДЕНЬ (СТРОГО ПРИДЕРЖИВАЙСЯ ЭТИХ ЦИФР):
• Калории: {goals['calories']} ккал (погрешность ±50 ккал)
• Белки: {goals['protein']} г (погрешность ±10 г)
• Жиры: {goals['fat']} г (погрешность ±5 г)
• Углеводы: {goals['carbs']} г (погрешность ±10 г)

ВНИМАНИЕ! Итоговые КБЖУ за день ДОЛЖНЫ быть максимально близки к этим цифрам.
Если текущих продуктов не хватает, чтобы закрыть цели — укажи это в разделе "ЧТО МОЖНО ДОКУПИТЬ" с конкретными продуктами и их количеством.
"""
...
СОСТАВЬ РАЦИОН НА СЕГОДНЯ строго по следующей структуре (каждая секция начинается с новой строки):
...
🛒 ЧТО НУЖНО ДОКУПИТЬ (если текущих продуктов не хватает для закрытия целей):
• продукт — количество (для чего нужен: белки/жиры/углеводы)
• продукт — количество (для чего нужен: белки/жиры/углеводы)
...
4. Если продуктов недостаточно для достижения целей по КБЖУ, конкретно укажи, что докупить и в каком количестве
5. НЕ используй символы * _ ` ~ для форматирования
6. Итоговые КБЖУ должны быть максимально близки к целям

ОТВЕЧАЙ ТОЛЬКО В УКАЗАННОМ ФОРМАТЕ. МЕЖДУ БЛОКАМИ ДОЛЖНЫ БЫТЬ ПУСТЫЕ СТРОКИ."""
    
    try:
        response = await deepseek.chat.completions.create(
            model="openai/gpt-oss-120b:free",
            messages=[
                {"role": "system", "content": "Ты — эксперт по здоровому питанию. Отвечай строго по структуре. Каждый приём пищи начинай с новой строки. НЕ используй символы * _ ` ~. Используй только эмодзи."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.6,  # чуть ниже для точности расчётов
            timeout=60.0
        )
        ...
        # Добавляем перенос перед каждым блоком приёма пищи
        meal_plan = re.sub(r'(🕗 ЗАВТРАК)', r'\n\n\1', meal_plan)
        meal_plan = re.sub(r'(🕛 ОБЕД)', r'\n\n\1', meal_plan)
        meal_plan = re.sub(r'(🕔 ПЕРЕКУС)', r'\n\n\1', meal_plan)
        meal_plan = re.sub(r'(🕘 УЖИН)', r'\n\n\1', meal_plan)
        meal_plan = re.sub(r'(📊 ИТОГО ЗА ДЕНЬ)', r'\n\n\1', meal_plan)
        meal_plan = re.sub(r'(🛒 ЧТО НУЖНО ДОКУПИТЬ)', r'\n\n\1', meal_plan)
        ...
        await message.answer(
            ...
            "Я запомнил этот рацион. Завтра предложу что-то новое!\n"
